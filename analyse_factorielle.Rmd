...
# Transposer les données pour avoir les observations en colonne
df_t <- t(df_scaled)

# Initialiser la PCA incrémentale avec la première observation
init_obs <- df_t[, 1:3]
xbar <- colMeans(init_obs)
init_pca <- list(xbar = colMeans(init_obs), n = 3)

# Appliquer la PCA incrémentale aux autres observations
result <- Reduce(incremental_pca, df_t[, -1], init = pca)
