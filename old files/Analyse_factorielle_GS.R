...
# Pour réaliser l'analyse factoriele, on travaille sur un dataframe. On convertit donc la base
# d'étude sous forme dedatatable en dataframe:
...
# On peut réduire à un département: le 93 par exemple..., au hasard!
...
# On commence par procéder à l'agrégation des données par département:

# on crée un compteur:
test2$compteur <-1

test3_dep <- test2[ , .(
  Nb_communes = sum(compteur, na.rm = TRUE),
...
resultats_acp_dep <- PCA(test3_dep_df_acp[,2:11], scale.unit = TRUE, ncp = 5, graph = TRUE)
summary(resultats_acp_dep)

dimdesc(resultats_acp_dep)

plot(resultats_acp_dep,select="cos2 0.7")

res = PCAshiny(test3_dep_df_acp)
