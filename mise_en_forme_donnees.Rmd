...
```{r, include=FALSE}
list.of.packages <- c("tidyverse", "sf", "readxl", "rgdal", "ggplot2", "data.table", "dplyr", "stargazer", "cartography","sf","rgdal","sp","readr", "sp", "stringr", "questionr",'geometry')
...
zonages <- read.csv("M:/Commun/BESTRP/Territoires/Zonage.StatApp/test.code/stats_app-main/stats_app-main/table-appartenance-geo-communes-22.csv",sep=';',fileEncoding='cp1252')

```

```{r chargement-régions}
regions <- read.csv('departements-region.csv') 
#Error in file(file, "rt") : impossible d'ouvrir la connexion sur mon ordinateur(clem)
#c'est pour le mosaic plot dans statsdesc qu'on charge ça?
```
