...
save(donnees_joint_inf, file = "del_merged.RData")
rm(t.del)
```
...
decoupage_classe <- function(classe) {
  ...
  saveRDS(donnees_joint_inf1, file = paste(paste('del_merged',classe,sep = "_"),'rds',sep='.')) # on enregistre le sous df correspondant
}
...
lapply(liste_classe2, decoupage_classe) # on applique la fonction à chaque classe pour créer le df correspondant
