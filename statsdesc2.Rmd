...
#me donne une erreur : Error in FUN(X[[i]], ...) : aucun package nommé ‘ggcorrplot’ n'est trouvé
```
...
table(t.del, t.del$classe) ##Attention fait planter mon ordi (Clémence)
# Erreur dans xtfrm.data.frame(x) : (converti depuis l'avis) impossible d’utiliser xtfrm sur un tableau de données (data frame) (Gabriel)
```
