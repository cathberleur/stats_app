...
compteur_zonages <- function(zonage) {
  df_wide <- pivot_wider(data=t.del.K, names_from = 'info commune', id_cols = 'id', values_from = zonage)
  ...
  TRUE ~ 0)
  denominateur <- length(unique(t.del.K$id))
...
denominateur <- length(unique(df$id))
  ...
  cat(zonage, ' : ', effectif,', ', proportion, '%\n')
